...
if [ $? == 1 ]; then
    ...
    wp post create --post_author=admin --post_category="Test" --post-title="Test Title" --post-content="ft_services is a 2 week project" --post_excerpt=tag --post_status=publish | awk '{gsub(/[.]/, ""); print $4}' > /tmp/postid
